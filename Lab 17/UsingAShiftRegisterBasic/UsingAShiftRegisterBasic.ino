...
Example sketch 16
SHIFT REGISTER
At sometime or another you may run out of pins on your Arduino board 
and need to extend it with shift registers.
In this tutorial we are using a shift register to turn three pins 
into eight(or more outputs)
*/
// Pin definitions:
...
void loop()
{
  ...
  // you want to run, and comment out the remaining ones to disable them from running.
  //*********************************************************
  //*********************************************************
  //*********************************************************
 
  //oneAfterAnother();    // one after the other(ON)then one after the other(OFF)
  //oneOnAtATime();     // Scroll down the line on / off then start again
...
also lights them for random times? See http://arduino.cc/en/Reference/Random
